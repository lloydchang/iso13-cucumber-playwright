...
Given('I run a load test for {string} with {int} virtual users for {int} seconds', async function (endpoint: string, vus: number, duration: number) {
  const jsonReportPath = 'reports/performance/loadTest.json';
  const htmlReportPath = 'reports/performance/loadTest.html';
  ...
  const loadTestPath = path.resolve(__dirname, '../support/performance/loadTest.js');

  // Command to run k6 without hardcoding VUs and duration
...
Then('the test should complete successfully', function () {
  const resultData = JSON.parse(testResult);
  const checks = resultData.metrics.checks;

  if (checks && checks.values.rate < 1) {
    throw new Error(`Test did not complete successfully. Success rate was ${checks.values.rate * 100}%.`);
  } else {
    console.log('All checks passed successfully with a 100% success rate.');
  }
});

Then('the average response time should be below {int}ms', function (responseTimeThreshold: number) {
  const resultData = JSON.parse(testResult);
  ...
  if (httpReqDuration) {
    ...
    if (avgResponseTime >= responseTimeThreshold) {
      throw new Error(`Average response time ${avgResponseTime}ms exceeds threshold of ${responseTimeThreshold}ms.`);
    }
  } else {
...
Then('the success rate should be {int}%', function (successRate: number) {
  const resultData = JSON.parse(testResult);
  const checks = resultData.metrics.checks;

  if (checks && checks.values.rate * 100 < successRate) {
    throw new Error(`Success rate ${checks.values.rate * 100}% is below expected ${successRate}%.`);
  } else {
    console.log(`Success rate is ${checks.values.rate * 100}% and meets the expected ${successRate}%.`);
  }
});
